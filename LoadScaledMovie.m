...
% frames: [start end] vector of frames to return. End-start must be <= x, 
%   where x is the number of frames saved in each file, else it will return 
%   only the first x frames specified by the interval [start start+x-1].
%   This is so that you never load more frames than you wanted to in the
%   variable FrameLoadMax in smFRETsetup.
...
function [movRed,movGreen,movRedBkgnd,movGrBkgnd,lastframe] = LoadScaledMovie(PathToMovie,frames,varargin)
    ...
    if frames(2)>totfiles*FrameSaveIncr
        disp('LoadScaledMovie: Movie not that long?')
        movRed = -1;
        movGreen = -1;
        lastframe = -1;
