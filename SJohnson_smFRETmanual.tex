...
\caption{Screenshots for the movie scaling (data pre-processing) steps. (A) For every frame in a movie, the minimum (top panel, red), 99.99th percentile (top panel, blue) and median (bottom panel, green) intensity values are calculated and displayed.  % Obvious outliers in intensity maxima (corresponding to one very bright pixel in some frames) are excluded before the maximum intensity that will be set to 1 is computed.  
