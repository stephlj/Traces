    ...
    if ~isempty(DoMap)
        ...
        for i = 1:BdDir
            ...
            if debug
                ...
                ylim([-512 0])
                xlim([0 256])
                title('Red dot is center of point in starting channel')
                pause
                close all
                clear matchG_abs
            end
...
%%%%%%SECOND PART: Analyze data:
    ...
    for i = 1:length(ToAnalyze)
       ...
       % Find spots in both channels, but don't double-count:
        ...
        if debug
            figure
            PutBoxesOnImageV4(mat2gray(mean(TotImg(:,:,1:20),3)),[spotsG_abs';spotsRguess'],params.DNAsize);
        ...
        if debug
            figure
            PutBoxesOnImageV4(mat2gray(mean(TotImg(:,:,1:20),3)),[spotsG_abs';spotsRguess';spotsR'],params.DNASize);
            title('All spots found in red and green channels, and green matched to red','Fontsize',12)
            figure 
            PutBoxesOnImageV4(mat2gray(mean(TotImg(:,:,1:20),3)),spots',params.DNASize);
